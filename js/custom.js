	...
	if (post_code == '') {
		...
		success: function(data){
			...
			if (data.res.length > 0) {
				...
				if (parseFloat(item.distance).toFixed(2) < 50) {
				    str += '<li>';
...
/*******************Lesson Details Change***********************/
	...
	$('.lessonChange').removeClass('active');
	...
	$.ajax({
		...
		success: function(data){
			...
			str += '<div class="step_desc">';
			if (item.desc_type == 'text') {
			str += '<h2 class="slide_desc_content"><span>'+item.desc_content+'</span></h2>';
			}else{
			str += '<img src="'+_baseUrl+'uploads/lesson/'+item.desc_content+'">'; 
			}
			str += '</div>';
			str += '<h4 class="step_desc_text">'+item.other_content+'</h4>';
			str += '</div></div>';
			});
		    }
		    str += '</div></div>';
    ...
    $('.lessonChange.active').trigger("click");
