...
#########################################################
	...
	{
            $this->db->select('*');
            $this->db->from('dt_mock_answer_master');
...
#########################################################
        ...
        function getAnswersDB($condition_arr=array())
        {
            ...
            $this->db->select('AM.*');
            $this->db->from('dt_mock_answer_master AM');
	    $this->db->join('dt_mock_question_master QM', 'AM.mock_question_id = QM.mock_question_id');
            $this->db->where($condition_arr);
            $query=$this->db->get();
...
#########################################################
        ...
        function addAnswersDB($insert_arr=array(),$condition_arr=array()) {
            ...
            $this->db->where($condition_arr);
            $this->db->delete('dt_mock_answer_master');
            
            $this->db->insert_batch('dt_mock_answer_master', $insert_arr);
            return TRUE;
            }
