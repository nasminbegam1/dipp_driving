...
/******************Reset Password ****************************/
	...
	$profile_dtls 		= $this->model_instructor->getProfileId(array('instructor_id' =>$instructor_id,'profile_status !='=>'canceled'));
	if(is_object($profile_dtls)){
	
	$pp_profile_status          = $pp_profile->manageProfileStatus($profile_dtls->profile_id,'Cancel', 'Profile cancelled at store');
	    if($pp_profile_status['ACK'] == 'Failure'){
		$this->session->set_flashdata('message', array('title'=>'Instractor','content'=>$pp_profile_status['L_LONGMESSAGE0'],'type'=>'errormsgbox'));
	    }else{
